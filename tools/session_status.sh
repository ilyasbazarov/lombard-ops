...
# Девять счётчиков, каждый — число плюс совпавшие строки. Счётчик 9 (ADR-085) в общий
# вердикт не входит: он предупредительный. Вердикт положительным счётчиком,
...
# --- 9. Гейт горячего состояния и цена обязательного контекста (ADR-085) ------
# В общий вердикт НЕ входит и остановки не требует: пункт «Подробностей» снимается
# ТЕМ ЖЕ коммитом, которым закрывается его якорь, поэтому в момент проверки признак
# неизбежно срабатывает на законном содержании — блокировать он права не имеет
# (норма ADR-074 п.7). Печатает числа и совпавшие строки; решение о вытеснении
# принимает сессия, а не скрипт.
echo "-- 9. Горячее состояние и цена обязательного контекста (ADR-085) --"
if [ -x tools/state_hot_check.sh ]; then
    bash tools/state_hot_check.sh
else
    echo "count=НЕТ ЗАМЕРА — tools/state_hot_check.sh отсутствует или не исполняем"
fi
echo

#!/usr/bin/env python3
"""ЗАМОРОЗКА `ADR-061` СНЯТА (2026-08-20, `T-2-4`, шаг 1) — оба условия снятия закрыты.

Заморозка держалась на двух гейтах, названных ею же: `Q-25` (где живут артефакты первичного
аудита и что из них переносится в контур) — закрыт `T-0-20`, карта аудита от 2026-08-18; `Q-26`
(кто проставляет начальный набор скупочных цен до даты) — закрыт `ADR-064`, стартовый сид
синтетический с обязательной меткой `price_source`. Схема этого скрипта (формат CSV, путь, шесть
колонок листа «Справочник») больше не висит на неопределённом объекте «данные Блока A»: источник
и место ведения справочника — лист «Справочник» в Google Sheets, назначено владельцем и записано
`ADR-060`. Снятие текста заморозки применяет уже принятые решения `ADR-064`/`ADR-069`/`T-0-20`, не
создаёт нового.

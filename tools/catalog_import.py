#!/usr/bin/env python3
"""ЗАМОРОЖЕН `ADR-061` — К ИСПОЛЬЗОВАНИЮ ЗАПРЕЩЁН до закрытия `Q-25` и `Q-26`.

ПРИЧИНА ЗАМОРОЗКИ, названная прямо. Схема этого скрипта — формат CSV, путь, шесть колонок, порядок
передачи — НЕ ВЗЯТА НИ ИЗ ОДНОГО АРТЕФАКТА. Она была назначена сессией вокруг объекта «данные
Блока A», устройства которого сессия не знала: Блок A есть завершённый блок первичного аудита
бизнеса (аппендикс `APP_A_portfolio.md` в пирамиде артефактов аудита), а не файл и не выгрузка, и
справочник в нём СИНТЕЗИРОВАН моделью как образец. Четыре зелёных прогона ниже подлинные, но они
меряли выдуманную схему против самой себя — это и есть класс отказа, ради которого заведено правило
«неопределённый объект» (`05 §I`).

Скрипт не удалён намеренно: работа предъявлена, логика нормализации (класс ликвидности, проценты,
суммы) переживёт смену источника. Использовать его можно только после того, как объект определён
строкой реестра внешних источников (`11_INFRA_FACTS`), а схема подтверждена артефактом.

...
| Пользователь `LOMBARD_RO` | существует, пароль известен владельцу (подтверждено подключением); свои гранты — `SELECT` на 9 таблиц из `briefs/T-0-5.md`. **НЕ read-only фактически.** Измерено `T-0-5`, 2026-08-11: `PUBLIC` несёт полный набор `D/I/R/S/U` на этих 9 таблицах и на одной проверенной вне списка, а `SELECT` — на 107 таблицах базы (всего таблиц 108 по замеру `T-0-2`); права записи на остальных 98 не измерены. Гранты `PUBLIC` не отзываются (`ADR-032`). **Компенсирующий контроль «транзакция только на чтение» НЕ ИЗМЕРЕН** (`ADR-033`, вердикт «опровергнут» снят): проба шага 6-е с предикатом `WHERE 1 = 0` не различает исходы — обе пробы, в обычной транзакции и в режиме `SET TRANSACTION READ ONLY`, дали одинаковую тишину; лог целиком — `reference/T-0-5_readonly_tx_measurement_2026-08-11.md`. Уровень отказа в движке меряется на НАШЕМ экземпляре (`T-0-17`, `Q-17`), на базе клиента решающая проба запрещена `00 §4`. До предъявления фактов учётка не используется как единственная защита от записи; основной контроль — белый список операторов в нашем слое. Первый замер: `reference/T-0-5_readonly_measurement_2026-08-11.md` | вне репо (пароль) |

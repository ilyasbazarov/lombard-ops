...
| Канал прод-подключения | **агент на сервере клиента** (вариант Б `01 §4`) — решение владельца 2026-08-12, `ADR-046`; `Q-3` закрыт. Только исходящий трафик, входящие соединения к контуру клиента не нужны. Реализация — `T-0-8`. **Связность ПРОВЕРЕНА 2026-08-12 и канал пригоден** (`ADR-048`, шаг 1 задачи): с сервера ERP исходящий HTTPS доходит до `storage.googleapis.com` (корректный сертификат во всех прогонах) и до `oauth2.googleapis.com` (имя покрыто `*.googleapis.com` в списке `subjectAltName`, штатный валидатор .NET сертификат принимает, `POST /token` возвращает штатную ошибку Google). Прокси не задан ни в WinHTTP, ни в WinINet; `hosts` пуст; доверенных корней в `LocalMachine\Root` — 42; Windows Server 2019 build 10.0.17763. Первый вердикт «не пройдено» был ложноотрицательным — проба сравнивала `Subject` вместо списка имён. Замеры: `reference/T-0-8_agent_channel_measurement_2026-08-12.md` (снятый вердикт, дословен), `reference/T-0-8_channel_remeasurement_2026-08-12.md` (действующий). **Не измерен `secretmanager.googleapis.com`** — закрывается внутри `T-0-8` до шага 5 |

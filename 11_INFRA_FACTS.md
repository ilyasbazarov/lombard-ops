...
| Пользователь `LOMBARD_RO` | существует, пароль известен владельцу (подтверждено подключением); свои гранты — `SELECT` на 9 таблиц из `briefs/T-0-5.md`. **НЕ read-only фактически.** Измерено `T-0-5`, 2026-08-11: `PUBLIC` несёт полный набор `D/I/R/S/U` на этих 9 таблицах и на одной проверенной вне списка, а `SELECT` — на 107 таблицах базы (всего таблиц 108 по замеру `T-0-2`); права записи на остальных 98 не измерены. Гранты `PUBLIC` не отзываются (`ADR-032`). **Компенсирующий контроль «транзакция только на чтение на клиентской стороне» измерен и ОПРОВЕРГНУТ**: `UPDATE` под `LOMBARD_RO` в режиме `SET TRANSACTION READ ONLY` прошёл без ошибки прав, замер `T-0-5`, 2026-08-11 — `reference/T-0-5_readonly_tx_measurement_2026-08-11.md`. Замена контроля не назначена, вопрос на пересмотре архитектора (`Q-16`). До решения учётка не используется как единственная защита от записи. Первый замер: `reference/T-0-5_readonly_measurement_2026-08-11.md` | вне репо (пароль) |

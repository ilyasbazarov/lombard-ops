...
- **Гранты этого пользователя read-only НЕ гарантируют** (измерено T-0-5, 2026-08-11): псевдо-пользователь `PUBLIC` несёт полный набор прав на используемых таблицах, и отзывать их мы не будем (ADR-032). Поэтому контракт производителя данных включает обязательный контроль: **любое подключение к базе PawnShop открывается транзакцией только на чтение** — это и есть механизм, отбивающий запись на стороне сервера.

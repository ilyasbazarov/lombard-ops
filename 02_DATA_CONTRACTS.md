...
| vehicle_make (марка) | **не найдено** | — | — | `T-0-6`, 2026-08-19: `LIKE`-поиск по фрагментам MARK/BRAND в `DIR_CUSTOM_FIELDS.NAME` не дал ни одного совпадения. Кандидат `ID_FIELD=16` (`SUBJ_TECH_MODEL`, `ID_OBJ_TABLE=9`) найден по фрагменту MODEL, не MARK/BRAND, и имеет 0 непустых значений из 28 строк — полем-источником не является (критерий приёмки `T-0-6`) |
| vehicle_year (год) | **не найдено** | — | — | `T-0-6`, 2026-08-19: `LIKE`-поиск по фрагментам YEAR/GOD в `DIR_CUSTOM_FIELDS.NAME` не дал ни одного совпадения |

**Чего в этой таблице НЕТ, и это названо, а не забыто (`ADR-083` п.6–8, п.10-в).** `vehicle_model` и
госномер найдены и заполнены строками выше (`T-0-6`, 2026-08-19); `vehicle_make` и `vehicle_year`
измерены и явно закрыты вердиктом «не найдено» — тем же замером, не молчанием. Строк для
...
модель и госномер подтверждены только теперь (`T-0-6`), марка и год записью 2026-07-14 не
подтверждаются вовсе — реальный замер их не нашёл.

...
| vehicle_year (год) | **источника нет** — `Q-31`, решение владельца | — | — | `T-0-6`, 2026-08-19: `LIKE`-поиск по фрагментам YEAR/GOD в `DIR_CUSTOM_FIELDS.NAME` не дал ни одного совпадения; в значениях год отдельной величиной тоже не стоит. Радиус узкий (`ADR-086` п.4): год — элемент показа (шапка карточки) и вход формы осмотра `T-2-5`, где его вводит человек; цену, светофор и конвейер он не держит. До решения колонка остаётся пустой, и это НЕ дефект наполнения |
...
источника не имеет вовсе и вынесен строкой `Q-31` владельцу — это названо вердиктом, не молчанием. Строк для

...
- Задача: `T-0-10` · класс A · коммит на входе `ccffb3fdaf363df7d93e9531b646aaebfb419b75` ·
  коммит на выходе `b9286e2138b7645b0ef90532fcbec7c3dee0533f`.
- Файлы: `connector/mapping.json` (новый), `connector/agent/schema_guard.py` (правка —
  точка выбора источника + докстринг, `ADR-080` п.2), `connector/agent/test_schema_guard.py`
  (новый, `ADR-080` п.3), `reference/T-0-10_mapping_fingerprint_2026-08-18.md` (этот файл).
- `INFRA_PATCH`: нет изменений — облачные ресурсы, БД клиента, GCP не затронуты; вся работа —
  локальный парсинг файла вне репозитория, код и mock-тесты.
- Незаконченное: нет — все пять пунктов приёмки предъявлены логами выше.

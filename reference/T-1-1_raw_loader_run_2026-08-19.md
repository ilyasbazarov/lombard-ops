...
## Шаг 10 — ЗАКРЫТ. Четвёртый ускоренный замер (2026-08-20, класс B, подтверждено владельцем) — SUCCESS

`scripts/T-1-1_deploy.sh part2` (пересборка с исправленным `guard.py` + прежний entrypoint-фикс)
исполнен целиком. Расписание `13:59` Asia/Bishkek (~6 минут), после замера возвращено на `0 4 * * *`.

**`raw-loader-nfbmg` — `Execution completed successfully in 52.81s`.** Источник вызова —
`cloudscheduler.googleapis.com` (`AttemptFinished httpRequest.status=200`), исполнитель job —
`lombard-pipeline@…` (`RUN BY`), человек не участвовал.

**Лог загрузчика — все девять таблиц, без расхождений:**
```
(д) сводка: 9 таблиц загружено без расхождений — {'CONTRACTS': 2521, 'CONTRACTS_TERMS': 2522,
'SUBJECTS': 2519, 'CUSTOM_FIELDS_VALUES': 12749, 'DIR_CUSTOM_FIELDS': 23, 'TABLES_TABLE': 17,
'CONTRACT_STATES': 6, 'DEPOSIT_TYPES': 7, 'OPERATIONS': 6587}
```

**Счёт строк BigQuery = счёт строк NDJSON, обе стороны, все девять:**
| таблица | `bq query COUNT(*)` | `wc -l` блоба | `MAX(loaded_at)` |
|---|---|---|---|
| raw_contracts | 2521 | 2521 | 2026-08-20 07:59:32 |
| raw_contracts_terms | 2522 | 2522 | 2026-08-20 07:59:32 |
| raw_subjects | 2519 | 2519 | 2026-08-20 07:59:32 |
| raw_custom_fields_values | 12749 | 12749 | 2026-08-20 07:59:32 |
| raw_dir_custom_fields | 23 | 23 | 2026-08-20 07:59:32 |
| raw_tables_table | 17 | 17 | 2026-08-20 07:59:32 |
| raw_contract_states | 6 | 6 | 2026-08-20 07:59:32 |
| raw_deposit_types | 7 | 7 | 2026-08-20 07:59:32 |
| raw_operations | 6587 | 6587 | 2026-08-20 07:59:32 |

**Состав колонок сходится с отпечатком `mapping.json`** — проверено `bq show` на `raw_contracts`:
27 полей, полный набор имён из шага 1 (включая `loaded_at`/`source_blob`), типы — `INTEGER`
(legacy-алиас `INT64`, guard теперь нормализует, шаг 11 отрицательной пробы это покрывает). Ни одна
колонка не добавлена, ни одна не потеряна.

Расписание возвращено на `0 4 * * *`. Шаг 10 закрыт наблюдением на ускоренном (не штатном) окне —
критерий брифа «источник не человек» соблюдён (`cloudscheduler.googleapis.com`), критерий
«по расписанию, без ручного `jobs run`» соблюдён (вызов инициировал Scheduler, не `gcloud run jobs
execute`). Штатное окно `04:00` Бишкек подтвердит то же самое без вмешательства следующей ночью.

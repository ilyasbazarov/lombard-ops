...
## Класс B — НЕ ИСПОЛНЕНО, ждёт карточек подтверждения владельца (шаги 7-10)

Ни одна облачная команда `scripts/T-1-1_deploy.sh` не запускалась. Девять таблиц `raw_*` в живом
датасете `lombard_ops` НЕ созданы, Cloud Run Job `raw-loader` НЕ задеплоен, Cloud Scheduler job
`raw-loader-trigger` НЕ создан — соответственно шаг 10 (первый автоматический прогон) недостижим до
исполнения карточек 1-3. Три отдельных сообщения с картами подтверждения — предмет отдельного шага
переписки с владельцем (не этого артефакта): что исполняется, на каком объекте, чем откатывается —
уже названо в `scripts/T-1-1_deploy.sh` докстрингом по каждой части.

...
if (-not $env:LOMBARD_DB_CHARSET) {
    Write-Error 'CONTEXT GAP: LOMBARD_DB_CHARSET ne zadan - agent otkazyvaetsya rabotat bez parametra podklyucheniya k Firebird do togo, kak on budet zapolnen vruchnuyu.'
    exit 2
}
$logDir = "C:\LombardAgent\logs"
if (-not (Test-Path $logDir)) { New-Item -ItemType Directory -Path $logDir | Out-Null }
$stamp = Get-Date -Format "yyyyMMdd_HHmmss"
$stdoutFile = Join-Path $logDir ("run_daily_{0}.out.log" -f $stamp)
$stderrFile = Join-Path $logDir ("run_daily_{0}.err.log" -f $stamp)
$proc = Start-Process -FilePath "C:\Program Files\Python314\python.exe" -ArgumentList '"C:\LombardAgent\code\connector\agent\run_daily.py"' -NoNewWindow -Wait -PassThru -RedirectStandardOutput $stdoutFile -RedirectStandardError $stderrFile
exit $proc.ExitCode
